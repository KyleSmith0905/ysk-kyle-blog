...
float colorManipulation(float colorChannel) {
  return mod(colorChannel * 0.5 + 0.5, 0.1) * 10.0;
}
...
void main() {
  ...
  float alpha = noise(vec3(x * 0.002, y * 0.002, u_time));

  gl_FragColor = vec4(u_color, colorManipulation(alpha) * u_opacity);
  return;
}
